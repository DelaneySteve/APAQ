...
class Airport:
    ...
    def _get_flights(
        self, endpoint: str, type: Literal["arrivals", "departures"], headers: dict[str, Any], page_size: int
    ) -> None:
        ...
        _logger.info(
            "Fetching %s for %s (%s) from %s... " "Page 1/%s",
            type,
            self.name,
            self.iata,
            endpoint,
            int(available_flights / page_size) + 1,
        )
        if available_flights > page_size:
            ...
            while available_flights >= current_flights:
                _logger.info(
                    "Fetching %s for %s (%s) from %s... " "Page %s/%s",
                    type,
                    self.name,
                    self.iata,
                    endpoint,
    ...
    def _fetch_next_page(
        self, endpoint: str, type: Literal["arrivals", "departures"], headers: dict[str, Any], page: int, retry: int = 1
    ) -> dict[Any, Any]:
    ...
    def _parse_flights(
        self, flights_data: list[dict[str, Any]], type: Literal["arrivals", "departures"]
    ) -> set[Flight]:  # pylint: disable=(line-too-long
        is_arrival = type == "arrivals"
        return {
            Flight.new_flight(self.iata, self.icao, flight_data=flight, is_arrival=is_arrival)
