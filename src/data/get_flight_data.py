...
REQUEST_HEADERS = {"user-agent": "Chrome/120.0.6099.129"}
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--get-airports", action="store_true", help="Use this flag to retrieve and save a list of all global airports."
    )
    parser.add_argument(
        "--airports-dataset",
        type=str,
...
def get_flights(airports: list[Airport]) -> None:
    for idx, airport in enumerate(airports):
        ...
        try:
            ...
            logger.error(
                "An error occurred when retrieving flight information for %s (%s).\n" "Airport index %s",
                airport.name,
                airport.iata,
                idx,
            )
...
def get_runways(airports: list[Airport]) -> None:
    for idx, airport in enumerate(airports):
        ...
        try:
            ...
            logger.critical(traceback.format_exc())
            logger.error(
                "An error occurred when retrieving runway information for %s (%s).\n" "Airport index %s",
                airport.name,
                airport.iata,
                idx,
            )
